					    ...
					    u32 *hw, unsigned int *count)
{
	...
	/*
	 * Pack the two fuse cells into the synthesized hw[0] word: bits
	 * [19:0] of hw[0] hold the jtag-id (from fuse[0]) and bits [27:20]
	 * hold the feat-id (from the low 8 bits of fuse[1]). These bit
	 * positions describe hw[0], not the layout of the underlying nvmem
	 * cells, which are read as independent full-width values.
	 */
	hw[0]  = fuse[0] & GENMASK(19, 0);
	hw[0] |= (fuse[1] & GENMASK(7, 0)) << 20;
	*count = 1;
...
static int tsens_register(struct tsens_priv *priv)
{
	...
	for (i = 0;  i < priv->num_sensors; i++) {
		...
		tzd = devm_thermal_of_zone_register_with_bin(priv->dev,
							     ...
							     &priv->sensor[i],
							     &tsens_of_ops,
							     priv->hw_bin_info.supported_hw_bin_count ?
